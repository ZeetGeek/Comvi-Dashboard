...
function SidebarLink({ icon, text, notification, submenu, navigate, submenu_links }) {
					...
					<>
						<button
							...
							<div className='flex items-center gap-[0.93rem]'>
								<Image src={icon.src} height={24} width={16} alt={icon.alt} />
								<span className='text-white text-lg capitalize font-semibold'>{text}</span>
							</div>

							{/* notification or submenu */}
							<div>
								<div className={` ${submenuOpen ? style.submenu_active : ""} h-[1.5rem] w-[1.5rem] rounded-[0.25rem] flex items-center justify-center `}>
									<Image src={SubmenuIcon} width={14} height={10} alt='submenu toggle icon' className='duration-300' />
								</div>
							</div>
						</button>
											...
											<>
												<li
													...
													className={`mb-4 ${style.submenu_links_wrapper} ${
														pathname === item.navigate ? style.submenu_parent_active : ""
													} `}>
													<Link
														href={item.navigate}
														alt={item.title}
						...
						<Link
							...
							<div>
								{typeof notification === "number" ? (
									<>
										<div className='h-[1.5rem] w-[1.5rem] bg-secondary rounded-[0.25rem] flex items-center justify-center'>
											<span className='text-sm text-white font-semibold'>{notification}</span>
										</div>
									</>
								) : (
									<></>
